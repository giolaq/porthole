...
export function createHttpServer(opts: HttpServerOptions) {
  ...
  const mjpegPollers = new Map<string, MjpegPoller>();
        ...
        async (serial) => {
          ...
          const result = await focusOn(
            ...
            async (button) => {
              ...
              if (handleInput) await handleInput(event, deviceId);
              else await getEngine(deviceId)?.sendInput(event);
            },
...
async function withSerial(
  res: ServerResponse,
  getDevice: (() => DeviceInfo | undefined) | undefined,
